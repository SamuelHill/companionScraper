...
def findTheNeedles(A, haystack):
	...
	if len(found): 
		for o in found:
			txt = re.sub(r'[\t /]+', '', o.title())+'-fieldOfStudy' 
			output.append(txt)
	return output

def print_subject(event_title, discipline_symbol, seeker, haystack):
	res = findTheNeedles(seeker, haystack)
	if len(res):
		print '(relevantTo '+event_title+' '+discipline_symbol+')'
		for r in res:
			print '(relevantTo '+event_title+' '+r+')'


def runEverythingThrough(ttle, abstract):
	print '(in-microtheory interestsMt)'
	event_title = re.sub(r'[\t /]+', '', ttle.title())+'-event'
	haystack = str((ttle + abstract).lower())
