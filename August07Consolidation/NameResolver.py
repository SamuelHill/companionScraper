...
class NameEntry:
	...
	def getMiddleNameAbbrsWithDots(self):
		return ' '.join([x[0] + '.' for x in self.middleNamesAbbrs])
	def getFirstNameAbbrDots(self):
		return '.'.join([x[0] for x in self.first.split('-')]) + '.'
...
def as_name_entry(dict):
	return NameEntry('', [str(x) for x in dict['divisions']], str(dict['first_name']), str(dict['last_name']), [str(x) for x in dict['middle_names']], [str(x) for x in dict['nick_names']])
...
class NameResolver:
	...
	def __init__(self, logger, path = ''):
		...
		if path == '':
			...
			for x in all_faculties:
				...
				self.addEntity(NameEntry( re.sub('[ \t]+', ' ', x['full_name']), []))
		else:
	...
	def printAllEntities(self):
		for key, value in self.lastNameDict.items() :
			for e in value:
...
def main():
	logger = SimpleLogger()
	r = NameResolver(logger, './intermediate_results/name_resolver.jl')
	r.printAllEntities()
	r.printLogs()
