...
class umsc:
	manual_loc = './intermediate_results/eecs-undergraduate-manual.pdf'
	manual_url = 'https://www.mccormick.northwestern.edu/eecs/documents/undergraduate/eecs-undergraduate-manual.pdf'
	manual_txt = './intermediate_results/ug_manual_text.txt'
	manual_pdf = './intermediate_results/eecs-undergraduate-manual.pdf'
...
class UndergraduateManualScraper:
	...
	def __init__(self):
		if not os.path.isfile(umsc.manual_loc):
			urllib.request.urlretrieve(umsc.manual_url, umsc.manual_loc)
	...
	def encodeCourse(self, string, discipline): 
		...
		for i in re.finditer(UndergraduateManualScraper.CoursePattern, string):
			...
			# trail = unidecode(str(string[elend:], encoding = "utf-8"))
			trail = str(string[elend:])
			trail = re.sub('([(].+[)]) *','',trail)
			trail = re.sub('- ','',trail)
